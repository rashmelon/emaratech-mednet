...
const ProvidersTable = (props) => {
    ...
    const providers = props.providers.map((provider, index) => {
        return (
            <ProviderListItem provider={provider} index={index} onRowClick={openRowHandler} openedIndex={openedIndex} rowIsOpened={openedIndex === index} />
        )
    });
    ...
    return (<div>
        ...
        {!props.isLoading && (
            ...
            <Pagination currentPage={props.currentPage} pagesCount={props.pagesCount} onPageChanged={props.onPageChanged}/>
        </div>
        )}
    </div>)
}
